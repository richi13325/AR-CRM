...
public class MensajeRepositoryAdapter
        implements SaveMensajePort, FindMensajesByConversacionIdPort, ExistsMensajeByWaMessageIdPort,
                   FindMensajeByWaMessageIdPort {

    private final MensajeRepository repository;

    @Override
    public Optional<Mensaje> findByWaMessageId(String waMessageId) {
        return repository.findByWaMessageId(waMessageId).map(MensajeMapper::toDomain);
    }
